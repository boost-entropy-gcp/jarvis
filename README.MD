...
---
## Actions
When the application runs, it completes three phases (also called actions). 
The first is the __initiation__ phase, the next is the test __execution__ and finally comes the __assertion__.
This order never changes and each phase starts only after the previous has finished.
This part of the documentation details the role of each action and how the events can be customized during the steps.

### Initiation Action
The initiation action prepares the environment for the upcoming test execution. 
...
If the __testSuite.json__ allows autodetect, then Jarvis parses the configuration for the initiation actions from the folder structure of the test case.
There are two different valid folder structures, that Jarvis can detect.
...
The content of the 'pre' folder dictates what happens during the initiation phase, while the 'assert' folder controls the assertion action.
The next folder under it must be named the same as the corresponding context's id.
Within that the files can have any name.

The initiation action can execute SQL/BQL scripts on BQ, SQL Server, MySQL or PostgreSQL.
It can also upload any file to SFTP server.
